...
"mediaType": 1, 
"thumbnail": m, 
"mediaUrl": "https://youtube.com/@YukinaVGR", 
"sourceUrl": "https://youtube.com/@YukinaVGR" }}}, { quoted: m }) }
//▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰//
switch (command) {
case "menu": {
const text12 = `
...
"BEGIN:VCARD",
"VERSION:3.0",
`FN:YukinaVGR🦅[${createSerial(1)}] ${contact.split("@")[0]}`,
`TEL;type=CELL;type=VOICE;waid=${contact.split("@")[0]}:+${contact.split("@")[0]}`,
"END:VCARD",
"", ].join("\n");
return vcard; }).join("");
...
"BEGIN:VCARD",
"VERSION:3.0",
`FN:YukinaVGR🦅[${createSerial(2)}] ${contact.split("@")[0]}`,
`TEL;type=CELL;type=VOICE;waid=${contact.split("@")[0]}:+${contact.split("@")[0]}`,
"END:VCARD",
"", ].join("\n");
return vcard; }).join("");
...
if (!isOwner) return reply(`Khusus YukinaVGR Aja`)
if (!isGroup) return m.reply(`Khusus Group Kontol`)
